"""
    注意，这是一个旧版的分支代码，它只用于复现实验报告文档中触发浏览器警告的内容
...
class http_proxy:
    ...
    def get_host_info(self, if_hook_host = True, detour_host = b'www.baidu.com'):
        """
        获取目标主机的ip和端口
        ...
        复现方法：
            使用同目录下的 old_proxy.py 文件启动代理服务器，然后
            把函数签名中的 if_hook_host = False 改为 if_hook_host = True 即可
        """
